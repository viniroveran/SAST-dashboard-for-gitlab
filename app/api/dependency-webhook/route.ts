...
import { TrivyReport, TrivyVulnerability } from '@/app/types/trivy';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const viewUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/dependency-scan/${entry.id}`;

    const discordWebhookUrl = process.env.DISCORD_WEBHOOK_URL;

    if (discordWebhookUrl) {
      const allVulnerabilities: TrivyVulnerability[] = (trivyReport.Results || [])
      ...
      const discordMessage = {
        ...
        embeds: [
          {
            title: `📦 New Dependency Scan Report for ${repoName}`,
    ...
    return NextResponse.json({ success: true, id: entry.id, viewUrl }, { status: 201 });
  } catch {
